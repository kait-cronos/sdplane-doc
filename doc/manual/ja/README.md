...
6. [RIB・ルーティング](routing.md) - RIBとルーティング機能
   - [仮想スイッチ（vswitch）](vswitch.md) - 仮想スイッチの設定と管理（16コマンド）
   - [ネイバーテーブル（ARP/ND）](neighbor.md) - ネイバーテーブルの表示
   - [FDB（転送データベース）](fdb.md) - FDBの表示
   - [SRv6](srv6.md) - SRv6 Local SIDの設定と表示
...
1. **ポート管理・統計** (12コマンド) - DPDKポートの制御と統計
2. **ワーカー・lcore管理** (7コマンド) - ワーカースレッドとlcoreの管理
3. **デバッグ・ログ** (2コマンド) - デバッグとログ機能
4. **VTY・シェル管理** (4コマンド) - VTYとシェルの制御
5. **システム情報・監視** (9コマンド) - システム情報と監視
6. **RIB・ルーティング** (28コマンド) - ルーティング情報の管理（SRv6含む）
...
12. **rte-flow** (5コマンド) - ハードウェアフロー分類の設定
