[トップ](../../../README.md) > [ユーザーガイド](README.md) > シナリオガイド > 拡張リピーターアプリケーション

# 拡張リピーターアプリケーション

**Language:** [English](../en/enhanced-repeater-application.md) | **日本語**

enhanced-repeaterは、vswitch設定を基準に受信フレームをVLAN単位で分離して転送する拡張リピーター機能です。

## 概要

l2-repeaterのような「受信ポート以外の全ポート」複製ではなく、受信フレームが所属するvswitchを決定したうえで、同一vswitchに属するリンクだけへ中継します。各送信リンクの設定に応じて、VLANタグの付与・剥離・変換を行えます。

また、TAPデバイスへのフレームコピーが可能で、これを用いてLinux側でのパケット観測や、L3転送のオフロードが実現できます。
