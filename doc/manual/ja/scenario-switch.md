[トップ](../../../README.md) > [ユーザーガイド](README.md) > シナリオガイド > スイッチを使う
...
`l2-switch` ワーカーはFDBベースのVLAN内L2スイッチングを提供します。

- **ブロードキャスト/マルチキャスト**: VLAN内フラッディング
- **ユニキャスト**:
  - 宛先MACがFDBに存在 → 単一ポート転送
  - 未学習 → VLAN内フラッディング
- **router-if / capture-if が設定されている場合**: 同時にそれぞれへコピー
...
# lcoreワーカー設定
# enhanced-repeater: vswitchベースのVLAN中継（フラッディングのみ）
# l2-switch: FDBベースのL2スイッチング（MACアドレス学習による単一ポート転送）
